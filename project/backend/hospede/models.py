...
class Hospede(Database):
    __tablename__ = 'HOSPEDE'
    __tablecolumns__ = ['ID_HOSPEDE', 'NU_CPF', 'NO_HOSPEDE', 'NU_TELEFONE', 'NO_EMAIL', 'ID_ENDERECO']

    def insert(self, nu_cpf: str, no_hospede: str, nu_telefone: str, no_email: str, id_endereco: int):
        self.cursor.execute(f'INSERT INTO "{self.__tablename__}" '
                            f'VALUES (NULL, "{nu_cpf}", "{no_hospede}", "{nu_telefone}", "{no_email}", {id_endereco}) ;'.upper())
...
class HospedeEnderecoView(Database):
    __tablename__ = 'VW_HOSPEDE_ENDERECO'
    __tablecolumns__ = ['ID_HOSPEDE', 'NU_CPF', 'NO_HOSPEDE', 'NU_TELEFONE', 'NO_EMAIL',
